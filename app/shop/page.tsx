...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    if (category === "coffee" || category === "pastry") {
  ...
  const handleCategoryChange = (category: Category) => {
    setSelectedCategory(category);
    if (category !== "all") {
      router.push(`/shop?category=${category}`, undefined);
    } else if (category === "all") {
      router.push(`/shop?category=all`, undefined);
    } else {
    ...
    <div className="min-h-screen bg-white">
      <header className="fixed top-0 left-0 right-0 bg-white shadow-sm z-50">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <Link
            href="/"
            className="text-2xl font-serif font-bold text-espresso-700"
          >
            Crema Coffee House
          </Link>
          <button onClick={() => setIsCartOpen(true)} className="relative p-2">
            ...
            {cartItems.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-espresso-700 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
